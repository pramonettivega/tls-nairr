...
from tool import ConfigTLS as cfg
...
class S3DIS:
    ...
    def load_sub_sampled_clouds(self, sub_grid_size, labeled_point, retrain):
        ...
        for i, file_path in enumerate(self.all_files):
            ...
            """with open(join(join(self.path, self.name), (cloud_name)[:6])+'/'+(cloud_name)[7:]+'/unlabel.txt', 'r') as f:
                lines = f.readlines()
                idx_without_anno = [int(line.split(' ')[0]) for line in lines]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, default='train', help='options: train, test, vis, continue-train, transfer')
    ...
    parser.add_argument('--epoch', default=0, help='# of epoch to start training')
    ...
    if Mode == 'train':
        ...
        model.train(dataset)
    elif Mode == 'continue-train':
        # If training stops - get the saving path and continue from a specified epoch
        snap_path = join(cfg.saving_path, 'snapshots')
        snap_steps = [int(f[:-5].split('-')[-1]) for f in os.listdir(snap_path) if f[-5:] == '.meta']
        chosen_step = np.sort(snap_steps)[-1]
        chosen_snap = os.path.join(snap_path, 'snap-{:d}'.format(chosen_step))
        model = Network(dataset, cfg, epoch = FLAGS.epoch, restore_snap={"snap":chosen_snap, "step":chosen_step})
        model.train(dataset)
    elif Mode == 'transfer':
        # If model is using transfer-learing: Set all of the learnable layers to false taking the snapshot from the path set in saving_path
        snap_path = join(cfg.saving_path, 'snapshots')
        snap_steps = [int(f[:-5].split('-')[-1]) for f in os.listdir(snap_path) if f[-5:] == '.meta']
        chosen_step = np.sort(snap_steps)[-1]
        chosen_snap = os.path.join(snap_path, 'snap-{:d}'.format(chosen_step))
        model = Network(dataset, cfg, epoch = FLAGS.epoch, restore_snap={"snap":chosen_snap, "step":chosen_step}, transfer=True)
        model.train(dataset)
    elif Mode == 'test':
        # Test on a given dataset, snapshot taken from the specified saving path.
        cfg.saving = False
