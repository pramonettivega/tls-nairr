...
class ModelTester:
    def __init__(self, model, dataset, restore_snap=None):
        ...
        if restore_snap is not None:
            ...
            # Added for review:
            """wc_r1 = tf.get_default_graph().get_tensor_by_name('layers/fc/weights:0')
            print(wc_r1)
            print(wc_r1.eval(session=self.sess))
            a = 1/0"""
    ...
    def test(self, model, dataset, gen_pesudo=None, num_votes=100):
        ...
        # Test saving path
        saving_path = time.strftime('vol-aug/results/Log_%Y-%m-%d_%H-%M-%S', time.gmtime())
