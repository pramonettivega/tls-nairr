...
dataset_path = "C:/Users/pramonettivega/Desktop/sample-annotations"
##dataset_path = './vol/data/TLS_scans/TLS_scans'
# dataset_path = './vol-aug/data/TLS_scans/TLS_scans'
# dataset_path = '/media/qy-sysu/data/Dataset/S3DIS/Stanford3dDataset_v1.2_Aligned_Version'
# dataset_path = './vol/data/S3DIS/Stanford3dDataset_v1.2_Aligned_Version'
#anno_paths = [line.rstrip() for line in open(join(BASE_DIR, 'meta/anno_paths2.txt'))]
#anno_paths = [join(dataset_path, p) for p in anno_paths]
anno_paths = []

for root, dirs, files in os.walk(dataset_path):
    # Check if 'Annotations' is a directory in the current root
    if 'Annotations' in dirs:
        # Construct the full path to the 'Annotations' directory
        path = os.path.join(root, 'Annotations')
        consistent_path = path.replace("\\", "/")
        # Add the path to the list
        anno_paths.append(consistent_path)


gt_class = [x.rstrip() for x in open(join(BASE_DIR, 'meta/class_names_TLS.txt'))]
...
os.makedirs(sub_pc_folder) if not exists(sub_pc_folder) else None
out_format = '.ply'
...
def convert_pc2ply(anno_path, save_path):
    ...
    for f in glob.glob(join(anno_path, '*.txt')):
        ...
        pc = pd.read_csv(f, header=None, sep=r'\s+', engine='python').values
